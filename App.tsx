...
const MainApp: React.FC<{ navigate: (path: string) => void }> = ({ navigate }) => {
  ...
  return (
    <CourseProgressContext.Provider value={progressHook}>
      <div className="min-h-screen flex flex-col bg-slate-100">
        <Header onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} navigate={navigate} />
...
const Redirect: React.FC<{ to: string; navigate: (path: string) => void; message?: string }> = ({ to, navigate, message }) => {
  useEffect(() => {
    navigate(to);
  }, [to, navigate]);

  return <div className="flex justify-center items-center h-screen"><div>{message || 'Redirigiendo...'}</div></div>;
};
...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    const seedSubmoduleMedia = async () => {
      ...
      // Seed Video for 6-1
      const videoSeedFlag_6_1 = `seeded_video_preview_v6_${submoduleId_6_1}`;
      if (!localStorage.getItem(videoSeedFlag_6_1)) {
        try {
          const rawUrl = 'https://youtu.be/Nq66Wwm-wZ4';
          const embedUrl = getEmbedUrl(rawUrl);
          if (embedUrl) {
            await saveVideoUrl(submoduleId_6_1, embedUrl);
          }
        } catch (error) {
          console.error(`Failed to seed video database for ${submoduleId_6_1}:`, error);
        } finally {
          localStorage.setItem(videoSeedFlag_6_1, 'true');
        }
      }
      ...
      // Seed Video for 7-1
      const videoSeedFlag_7_1 = `seeded_video_preview_v6_${submoduleId_7_1}`;
      if (!localStorage.getItem(videoSeedFlag_7_1)) {
        try {
          const rawUrl = 'https://youtu.be/PvonOTKijfg';
          const embedUrl = getEmbedUrl(rawUrl);
          if (embedUrl) {
            await saveVideoUrl(submoduleId_7_1, embedUrl);
          }
        } catch (error) {
          console.error(`Failed to seed video database for ${submoduleId_7_1}:`, error);
        } finally {
          localStorage.setItem(videoSeedFlag_7_1, 'true');
        }
      }
      ...
      // Seed Video for 10-1
      const videoSeedFlag_10_1 = `seeded_video_preview_v6_${submoduleId_10_1}`;
      if (!localStorage.getItem(videoSeedFlag_10_1)) {
        try {
          const rawUrl = 'https://youtu.be/X6-eSyQu9jw';
          const embedUrl = getEmbedUrl(rawUrl);
          if (embedUrl) {
            await saveVideoUrl(submoduleId_10_1, embedUrl);
          }
        } catch (error) {
          console.error(`Failed to seed video database for ${submoduleId_10_1}:`, error);
        } finally {
          localStorage.setItem(videoSeedFlag_10_1, 'true');
        }
      }
  ...
  if (route.startsWith('/admin')) {
    if (currentUser.role === 'superadmin' || currentUser.role === 'admin') {
      ...
      return <Redirect to="/" navigate={navigate} message="No autorizado. Redirigiendo..." />;
    }
  }
